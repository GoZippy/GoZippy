...
- Decorative SVG motion is slow and ambient rather than pointer- or scroll-driven. Reduced-motion visitors receive a separately generated static hero; all animated SVGs also disable animation under that preference.
...
Before publication, render on GitHub and check the image proxy's animation behavior on the actual profile. Local animation checks and Markdown sanitization checks do not establish public image-proxy playback. If needed, use `option-verse-portal-static.svg`; no information is lost.
